...
class FasterRCNNMultiheadPseudo(torch.nn.Module):
    def __init__(self, num_classes, num_channels, device, config, image_size=800):
        super(FasterRCNNMultiheadPseudo, self).__init__()
        ...
        if self.training:
            ...
            for target in targets:
                ...
                if len(new_target['boxes']) == 0:
                    ...
                    new_target['score_labels'] = torch.zeros((0,), dtype=torch.float32, device=device)
                else:
                    new_target['labels'] = target['labels'][valid_indices]
                    new_target['score_labels'] = target['score_labels'][valid_indices]
...
def assign_targets_to_proposals(self, proposals, gt_boxes, gt_labels, gt_scores):
    matched_idxs = []
    labels = []
    scores = []
    for proposals_in_image, gt_boxes_in_image, gt_labels_in_image, gt_scores_in_image in zip(proposals, gt_boxes, gt_labels, gt_scores):
...
def select_training_samples(self, proposals, targets):
    ...
    matched_idxs, labels, scores = self.assign_targets_to_proposals(proposals, gt_boxes, gt_labels, gt_scores)
